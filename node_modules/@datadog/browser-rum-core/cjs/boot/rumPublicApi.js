...
const startRum_1 = require("./startRum");
function makeRumPublicApi(recorderApi, profilerApi, options = {}) {
    ...
    let strategy = (0, preStartRum_1.createPreStartStrategy)(options, trackingConsentState, customVitalsState, (configuration, deflateWorker, initialViewOptions, telemetry, hooks) => {
        ...
        const startRumResult = (0, browser_core_1.mockable)(startRum_1.startRum)(configuration, recorderApi, profilerApi, initialViewOptions, createEncoder, trackingConsentState, customVitalsState, bufferedDataObservable, telemetry, hooks, options.sdkName);
    ...
    const startView = (options) => {
        const handlingStack = (0, browser_core_1.createHandlingStack)('view');
    ...
    const rumPublicApi = (0, browser_core_1.makePublicApi)({
        ...
        startResource: (0, browser_core_1.monitor)((url, options) => {
            // Check feature flag only after init; pre-init calls should be buffered
            if (strategy.initConfiguration && !(0, browser_core_1.isExperimentalFeatureEnabled)(browser_core_1.ExperimentalFeature.START_STOP_RESOURCE)) {
                return;
            }
            // addTelemetryUsage({ feature: 'start-resource' })
            strategy.startResource((0, browser_core_1.sanitize)(url), {
                type: (0, browser_core_1.sanitize)(options && options.type),
                method: (0, browser_core_1.sanitize)(options && options.method),
                context: (0, browser_core_1.sanitize)(options && options.context),
                resourceKey: options && options.resourceKey,
            });
        }),
        stopResource: (0, browser_core_1.monitor)((url, options) => {
            if (strategy.initConfiguration && !(0, browser_core_1.isExperimentalFeatureEnabled)(browser_core_1.ExperimentalFeature.START_STOP_RESOURCE)) {
                return;
            }
            // addTelemetryUsage({ feature: 'stop-resource' })
            strategy.stopResource((0, browser_core_1.sanitize)(url), {
        ...
        startDurationVital: (name, options) => {
            const handlingStack = (0, browser_core_1.createHandlingStack)('vital');
            return (0, browser_core_1.callMonitored)(() => {
